...
function Sliders() {
  ...
  // Renderizado de un carousel multi-item
  // Render dinámico según tamaño de pantalla
const renderCategoria = (categoria) => {
  const itemsPerSlide = window.innerWidth < 600 ? 1 : 7;

Array.from({ length: Math.ceil(categoria.imagenes.length / itemsPerSlide) })
  const totalSlides = Math.ceil(categoria.imagenes.length / itemsPerSlide);
    ...
    <div key={categoria.id} className="mb-5 text-center">
      ...
      <div id={categoria.id} className="carousel slide" data-bs-touch="true">
        <div className="carousel-inner">
          {Array.from({ length: totalSlides }).map((_, grupoindex) => {
            const start = grupoindex * itemsPerSlide;
            const end = start + itemsPerSlide;
            const grupo = categoria.imagenes.slice(start, end);

            return (
              <div
                key={grupoindex}
                className={`carousel-item ${grupoindex === 0 ? "active" : ""}`}
              >
                <div
                ...
                >
                  {grupo.map((img, i) => (
                    <img
                      ...
                      style={{
                        width: window.innerWidth < 600 ? "90vw" : "250px",
                        height: window.innerWidth < 600 ? "auto" : "300px",
                        objectFit: "cover",
                        flex: "0 0 auto",
                      }}
                      alt=""
